...
# constants
CHART_CATEGORIES = ["nicht ausreichend", "ausreichend",
                    "befriedigend", "gut", "sehr gut", "keine Daten verfügbar"]
CHART_COLORS = ["#e74c3c", "#e67e22",
                "#3498db", "#f1c40f", "#2ecc71", "#ffffff"]
...
def build_labels(grade_statistics: list[ModuleGradeStatistics]) -> SemesterStatistics:
    ...
    for module in grade_statistics:
        ...
        sum_of_participants = module.very_good + module.good + \
            module.satisfactory + module.sufficient + module.insufficient
...
async def generate_plot_data(module_number: int, module_title: str, limit_semesters=20) -> dict:
    ...
    limit_statistics_by_semester = statistics_by_semester[(
        -1*limit_semesters):]

    if len(limit_statistics_by_semester) == 0:
        raise ValueError(
            f"No data found for module number: {module_number}. Module might not exist (in the database).")
...
def draw_stacked_bars(ax: Axes, df_percent: pd.DataFrame, semester_df: pd.DataFrame, categories: list, colors: list, participants: list[int]):
    ...
    for idx, category in enumerate(categories):
        bars = ax.bar(
            semester_df, df_percent[category], bottom=bottom, label=category, color=colors[idx])
        if bottom is None:
...
async def plot_diagram_as_complex_file(data: dict, output_directory='data/plots') -> str:
    ...
    if data is None or len(data) == 0:
        raise ValueError(
            "No data provided for plotting. Data dictionary is empty or None.")
    ...
    output_filename = os.path.join(
        output_directory, f"{data['Modulnummer']}.png")
    ...
    draw_stacked_bars(
        ax, df_percent, df["Semester"], CHART_CATEGORIES, CHART_COLORS, participants)
    ...
    # y-axis scale higher than 100% to make space for average grade labels
    ax.set_ylim(0, 120)

    # Legend is outside to the right
    ax.legend(title="Bewertung", bbox_to_anchor=(
        1.01, 1), loc="upper left", fontsize=10)

    # Add average grade labels on top of bars
    for i, avg in enumerate(df["average_grade"]):
        ax.text(i, 112, f'Ø {avg:.2f}', ha='center',
                va='bottom', fontsize=9, color='black')

    # Add participant labels below average grade (N=0 when average_grade == 0)
    for i, (avg, part) in enumerate(zip(df["average_grade"], participants)):
        n_label = 0 if avg == 0 else part
        ax.text(i, 108, f'N={n_label}', ha='center',
                va='bottom', fontsize=9, color='black')

    # Source text
    currentTime = datetime.now().strftime("%d.%m.%Y %H:%M")
    source_text = f'Quelle: Klausurstatistik der Fakultät für Wirtschaftswissenschaften der FernUniversität in Hagen - https://www.fernuni-hagen.de/wirtschaftswissenschaft/studium/klausurstatistik.shtml - Grafik erstellt: {currentTime} Uhr)'
    ax.figure.text(0.99, 0.01, source_text, ha='right',
...
async def plot_all_statistics():
    ...
    for module in all_modules:
        ...
        logger.info(
            f"Created/Updated graphic for module number: '{module.number}' at '{file_name}'")
