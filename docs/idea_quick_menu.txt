...
# Persistent view: https://github.com/Rapptz/discord.py/blob/master/examples/views/persistent.py
...
class QuickMenu(commands.GroupCog, name="quickmenu", description="Dies ist ein Text"):
    ...
    @app_commands.command(name="diesisteintest", description="21Listet die Text Commands dieses Servers auf.")
    # @app_commands.default_permissions(administrator=True)
    async def cmd_diesisteintest(self, interaction: Interaction):
        await interaction.response.defer(ephemeral=True)
        title="Example title"
        key = "Wiinfo Pflichtmodule"
        ...
        msg = f"📚 **{title}**\nSeite 1 von {(len(modules)-1)//25 + 1}"

        await interaction.edit_original_response(content=msg, view=ModuleView(title, modules))
