...
class QuickMenu(commands.GroupCog, name="quickmenu", description="Dies ist ein Text"):
    ...
    async def cmd_pflichtmodule_wiwi(self, interaction: Interaction):
        ...
        module_categories = self.get_module_categories_of_parent_category(interaction.guild.categories, start_string)  
        ...
        for category in module_categories: 
            ...
            module_name = "".join(char for char in category.name.replace(module_number, "") if char.isascii()).strip()
            ...
            channel = self.get_channel_or_first_public_of_category(category, "diskussion-und-infos")
            if channel is None:
                continue
